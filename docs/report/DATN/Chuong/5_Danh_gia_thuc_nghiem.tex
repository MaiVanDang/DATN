...
Cần lưu ý rằng con số EER 21,0\% nói trên được đo trên một cụm người lạ cố định (user22--user26) với hàm chấm điểm cơ sở \textit{cos\_mean}. Như sẽ phân tích ở Mục~\ref{sec:scorer}, phần lớn mức lỗi này bắt nguồn từ việc chỉ xét một cụm người lạ cố định và từ hàm chấm điểm chưa tối ưu, chứ không phản ánh giới hạn của bản thân bộ mã hóa; ở đây con số chỉ được dùng để xếp hạng tương đối giữa 3 kiến trúc dưới cùng một điều kiện đánh giá.
...
Việc so sánh hàm quyết định được thực hiện trên bộ mã hóa CNN 1D đã được chọn ở các phần trước. Để xác nhận lựa chọn kiến trúc không bị thay đổi bởi hàm chấm điểm mới, đề tài kiểm tra lại 3 kiến trúc backbone dưới hàm \textit{cos\_znorm} và quan sát thấy CNN 1D vẫn cho EER tập mở thấp nhất, nhất quán với kết luận rút ra dưới hàm cơ sở.
