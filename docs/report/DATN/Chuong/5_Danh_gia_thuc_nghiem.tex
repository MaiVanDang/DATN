...
Để đánh giá toàn diện, đề tài thực hiện thí nghiệm trên 2 kịch bản. Trong kịch bản nhận dạng tập đóng, các danh tính dùng làm người lạ khi kiểm thử là những người đã xuất hiện trong quá trình huấn luyện backbone. Trong kịch bản nhận dạng tập mở, người lạ là những danh tính được giữ riêng, hoàn toàn không tham gia huấn luyện backbone, qua đó mô phỏng đúng tình huống hệ thống gặp một người chưa từng biết. Kịch bản tập mở phản ánh sát mục tiêu phát hiện người lạ của đề tài, trong khi kịch bản tập đóng cho thấy giới hạn trên về khả năng phân biệt khi danh tính đã được quan sát.

Mỗi kịch bản lại được thực hiện theo 2 cấu hình ngữ cảnh hoạt động đã chuẩn bị ở Chương 3, gồm cấu hình chỉ đi bộ và cấu hình toàn bộ 3 hoạt động. Với mỗi tổ hợp kịch bản và ngữ cảnh, 3 kiến trúc backbone được đánh giá trên 3 cấu hình dữ liệu là chỉ dùng nhánh quán tính, chỉ dùng nhánh tương tác và dung hợp 2 nhánh.

Đồ án sử dụng 2 giao thức đánh giá khác nhau cho 2 mục tiêu khác nhau. Phần so sánh kiến trúc và cấu hình ở Mục~\ref{sec:ketqua} được đo trên một cụm người lạ cố định gồm 5 người được giữ riêng (user22-user26), với mỗi cấu hình được chạy 5 lần (đổi cách chia phiên ngẫu nhiên) rồi lấy trung bình; giao thức này đủ để xếp hạng tương đối giữa nhiều cấu hình với chi phí tính toán thấp, nhưng vì chỉ dựa trên một cụm người lạ duy nhất nên giá trị EER tuyệt đối phụ thuộc mạnh vào cụm cụ thể đó và chưa phản ánh mức lỗi trung bình trên toàn bộ tập người dùng. Để bảng gọn, độ lệch chuẩn giữa 5 lần chạy được lược bỏ trong các bảng ở Mục~\ref{sec:ketqua}. Phần khảo sát hàm quyết định ở Mục~\ref{sec:scorer} dùng kiểm thử chéo \textit{leave-users-out} 6 vòng, mỗi vòng giữ một cụm 5 người lạ khác nhau và báo cáo kết quả dưới dạng trung bình kèm độ lệch chuẩn - ước lượng đáng tin hơn về mức lỗi thực sự vì không phụ thuộc vào một cụm cố định. Vì khác giao thức, các con số EER tuyệt đối giữa 2 mục không so sánh trực tiếp với nhau; Mục~\ref{sec:scorer} sẽ phân tích tường minh phần chênh lệch nào đến từ giao thức đánh giá và phần nào đến từ hàm chấm điểm.
...
Bảng~\ref{tab:closed_all} và Bảng~\ref{tab:closed_walk} trình bày kết quả ở kịch bản tập đóng lần lượt cho cấu hình toàn bộ 3 hoạt động và cấu hình chỉ đi bộ. Các kết quả trong mục này được đo trên một cụm người lạ cố định (user22--user26), lấy trung bình 5 lần chạy và dùng để xếp hạng tương đối giữa các cấu hình.

\begin{table}[H]
    \centering
    \caption{Kết quả tập đóng, cấu hình toàn bộ 3 hoạt động}
...
Ở kịch bản tập đóng, cả 3 kiến trúc đều đạt hiệu năng cao trên nhánh quán tính, với EER trong khoảng từ 2,7\% đến 4,8\% và đều thỏa mục tiêu dưới 5\% đặt ra ban đầu. Nhánh tương tác đạt EER quanh mức 10\%, kém hơn rõ rệt so với nhánh quán tính. Cấu hình dung hợp không vượt trội mà thậm chí thấp hơn nhánh quán tính đơn lẻ; với CNN 1D, dung hợp kéo EER từ 3,35\% lên 22,36\%. Như vậy, ở kịch bản tập đóng, nhánh quán tính là thành phần mạnh nhất và là nhánh duy nhất đạt mục tiêu dưới 5\%, trong khi nhánh tương tác còn yếu và dung hợp tuyến tính theo điểm số không cải thiện hiệu năng trong điều kiện dữ liệu hiện có. Kết quả này định hướng thiết kế của đề tài xoay quanh nhánh quán tính, với nhánh tương tác và cấu hình dung hợp giữ vai trò đối chứng; cấu hình triển khai cụ thể được chốt ở Mục~\ref{sec:luachon}.

\subsection{Kết quả ở kịch bản tập mở}

Bảng~\ref{tab:open_all} và Bảng~\ref{tab:open_walk} trình bày kết quả ở kịch bản tập mở, là kịch bản phản ánh sát mục tiêu phát hiện người lạ. Như đã nêu ở Mục~\ref{sec:thinghiem}, các con số trong mục này cũng được đo trên cụm người lạ cố định (user22--user26), lấy trung bình 5 lần chạy.

\begin{table}[H]
    \centering
    \caption{Kết quả tập mở, cấu hình toàn bộ 3 hoạt động}
    \label{tab:open_all}
    \begin{tabular}{|l|l|c|c|c|c|}
    \hline
    \textbf{Kiến trúc} & \textbf{Cấu hình} & \textbf{AUC} & \textbf{EER (\%)} & \textbf{FAR (\%)} & \textbf{FRR (\%)} \\
...
Do chỉ xét một cụm người lạ cố định, ước lượng tập mở ở Mục~\ref{sec:ketqua} phụ thuộc mạnh vào cụm được chọn và có thể không đại diện. Để có ước lượng đáng tin và không phụ thuộc một cụm cụ thể, phần này áp dụng giao thức kiểm thử chéo \textit{leave-users-out}: ở mỗi vòng, một nhóm 5 người dùng được giữ làm người lạ, các người dùng còn lại đóng vai trò cohort tham chiếu; quá trình lặp qua 6 vòng với các nhóm khác nhau và kết quả báo cáo dưới dạng trung bình kèm độ lệch chuẩn. Bảng~\ref{tab:scorer_open} trình bày kết quả tập mở ở cấu hình toàn bộ 3 hoạt động.
...
\begin{table}[H]
    ...
    \hline
    \textbf{Hàm quyết định} & \textbf{EER tập mở (\%)} & \textbf{EER tập đóng (\%)} \\
    \hline
    cos\_znorm & 10,83 $\pm$ 6,51 & 2,24 \\
    cos\_knn   & 10,84 $\pm$ 7,13 & 2,83 \\
    cos\_mean (cơ sở) & 14,72 $\pm$ 6,27 & 3,11 \\
    maha       & 23,12 $\pm$ 5,73 & 17,51 \\
    \hline
    \end{tabular}
\end{table}

Kết quả cho thấy 2 quan sát quan trọng.

Thứ nhất, hàm \textit{cos\_mean} dùng trong đánh giá ban đầu không phải là lựa chọn tốt nhất: cả \textit{cos\_znorm} và \textit{cos\_knn} đều cho tỉ lệ lỗi cân bằng tập mở thấp hơn, trong đó \textit{cos\_znorm} đạt EER trung bình $10,83\% \pm 6,51\%$ so với $14,72\%$ của \textit{cos\_mean}. Việc chuẩn hóa điểm theo cohort giúp hiệu chỉnh độ lệch riêng của từng hồ sơ, qua đó tách bạch tốt hơn giữa chủ sở hữu và người lạ.

Thứ hai, có thể tách bạch 2 nguồn ảnh hưởng tới EER tập mở nhờ Bảng~\ref{tab:scorer_open} giữ cố định giao thức leave-users-out và chỉ thay đổi hàm chấm điểm. Một mặt, hàm \textit{cos\_mean} khi chuyển từ một cụm người lạ cố định ($21,0\%$ ở Bảng~\ref{tab:open_all}) sang trung bình qua 6 cụm người lạ khác nhau của kiểm thử chéo giảm còn $14,72\%$; phần giảm này đến từ việc không còn phụ thuộc vào một cụm duy nhất vốn tình cờ khó, tức từ giao thức đánh giá. Mặt khác, dưới cùng giao thức đó, việc thay \textit{cos\_mean} bằng \textit{cos\_znorm} tiếp tục kéo EER từ $14,72\%$ xuống $10,83\%$; đây là phần cải thiện do hàm chấm điểm mang lại. Tuy nhiên, ngay cả với giao thức đánh giá đáng tin và hàm chấm điểm tốt nhất, EER tập mở vẫn ở mức khoảng $10$--$11\%$ với độ lệch chuẩn lớn cỡ vài phần trăm, cho thấy nhận dạng tập mở trên số danh tính hạn chế là một bài toán thực sự khó: chuẩn hóa điểm theo cohort cải thiện rõ so với hàm cơ sở nhưng chưa đưa được mức lỗi xuống thấp.
...
\begin{table}[H]
    ...
    \hline
    Đề tài & 26 người, 19 dòng máy, in-the-wild & Quán tính (triển khai) & Tập mở + tập đóng & EER $\approx$2,2\% (đóng) / $\approx$10,8\% (mở); 6 anchor, leave-users-out & On-device TFLite & Cử chỉ lắc \\
    \hline
    \end{tabularx}
\end{table}
